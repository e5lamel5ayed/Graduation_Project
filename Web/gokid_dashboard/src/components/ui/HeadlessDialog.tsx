    ...
    <Transition.Root show={isOpen} as={Fragment}>
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        <Transition.Child
          ...
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-[#000000a3] transition-opacity" />
        </Transition.Child>
...
export function HeadlessDialogActions({
  children,
  className = '',
}: {
  children: React.ReactNode;
  ...
  return (
    <div
      className={`mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3 ${className}`}
    >
      {children}
    </div>
  );
